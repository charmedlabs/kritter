...
class Kslider(Kcomponent):
    def __init__(self, **kwargs):
        ...
        slider = dcc.Slider(value=value, min=mxs[0], max=mxs[1], step=mxs[2], updatemode='drag', className="_nopadding")
        cols = [self.label,
            dbc.Col(slider, id=self.id_col, width=self.style['control_width']), 
            dbc.Col(str(value), id=self.id_val, width='auto', className="_nopadding"),
        ]
